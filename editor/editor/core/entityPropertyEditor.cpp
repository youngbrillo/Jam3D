...
jam::UUID jam::strawberry::editor::register_component_popups(EntityInspectionFunction func)
{
	UUID id = UUID();
...
void jam::strawberry::editor::unregister_component_popups(UUID id)
{
	ECPFRManager::Get().pop_up_functions.erase(id);
}

jam::UUID jam::strawberry::editor::register_component_editWidget(EntityInspectionFunction func)
{
	UUID id = UUID();
...
void jam::strawberry::editor::unregister_component_editWidget(UUID id)
{
	ECPFRManager::Get().widget_functions.erase(id);
}

namespace jam::strawberry {
	static bool InspectTransform2D(Entity& entity, jam::components::Transform3D& transform) {
...
void jam::strawberry::editor::inspect_entity_properties(bool* visible, Entity& e)
{
	using namespace jam::components;
	using namespace jam::strawberry::editor::functions;
	...
	if (ImGui::BeginPopup("AddComponent"))
	{
		...
		{
			AddComponentPopup<Transform3D>(e, "Transform3D");
			ImGui::EndMenu();
		}
	...
	RenderEComponentEditWidget<Transform3D>("Transform 2D", e, InspectTransform2D);
	RenderEComponentEditWidget<SceneCamera3D>("Scene Camera 2D", e, InspectSceneCameraComponent);
	RenderEComponentEditWidget<CameraEditorComponent>("Camera Pan", e, InspectCameraEditorComponent);
