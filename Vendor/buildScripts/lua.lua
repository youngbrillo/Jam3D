...
function library:get_dir()
    ...
    local installLocationFolder = installLocation .. "/lua-5.4";  -- "/".. libFolder;
...
function library:link_to(pre) 
    ...
    local linkDest = self.pathprefix .. "lua-5.4"; --libFolder;
...
function library:loadProject()
    ...
    --change install location slightly because the folder name is 'slightly' different for lua
            installLocationFolder = installLocation .. "/lua-5.4";
...
function library:generatePremakeScript()
    ...
    local local_dir = "."..self.installFolder .. "/lua-5.4"

    if self.debug_print then
        print ("   2) Generating Script (yaml-cpp)")
